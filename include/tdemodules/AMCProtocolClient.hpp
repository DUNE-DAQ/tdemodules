...
#pragma pack(pop)
  ...
  std::vector<uint8_t> send_rrq(const std::vector<uint8_t>& payload) { return send_request(RRQ, payload); }

  std::vector<uint8_t> send_wrq(const std::vector<uint8_t>& payload) { return send_request(WRQ, payload); }
